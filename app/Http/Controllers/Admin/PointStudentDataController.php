...
class PointStudentDataController extends Controller
{
    function DataPoint() {
        $userStudent = User::latest('name', 'ASC')->get();
        $PSViolations = PSViolation::latest()->get();
        $PSAwards = PSAward::latest()->paginate(10);
        return view('admin.point.data_point', compact('userStudent', 'PSViolations', 'PSAwards'));
    }
